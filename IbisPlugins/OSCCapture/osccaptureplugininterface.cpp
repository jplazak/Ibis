...
=========================================================================*/
// Thanks to Simon Drouin for his help with writing this class
...
void OSCCapturePluginInterface::OnUpdate(){
    ...
    if( p ){
        ...
        //Get Pointer State :  enum TrackerToolState{ Ok, Missing, OutOfVolume, OutOfView, Undefined };
...
bool OSCCapturePluginInterface::HandleKeyboardEvent( QKeyEvent * keyEvent ) {
    ...
    if( keyEvent -> key() == Qt::Key_Space ){
        ...
        //RESET CONDITION:  Set counters back to zero
        counter = 0;
        unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
        std::shuffle(trial.begin(), trial.end(), std::default_random_engine(seed));
